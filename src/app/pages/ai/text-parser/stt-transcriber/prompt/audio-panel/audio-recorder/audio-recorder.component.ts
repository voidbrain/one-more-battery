...
import { webmFixDuration } from '@services/utils/blob-fix';
...
@Component({
  ...
  standalone: true,
  imports: [TranslocoModule, IonButton],
})
export class AudioRecorderComponent {
  ...
  private silenceInterval?: ReturnType<typeof setInterval>;
    ...
    try {
      ...
      this.mediaRecorder.ondataavailable = async (event) => {
        ...
        if (this.mediaRecorder?.state === 'inactive' && mimeType === 'audio/webm') {
          ...
          const fixedBlob = await webmFixDuration(
      ...
      this.notificationService.showError(this.transloco.translate('ERR_ACCESSING_MICROPHONE'));
      console.error(error);
    }
  }
  ...
  private setupCanvas() {
    ...
    this.visualizer.nativeElement.width = this.soundClips.nativeElement.offsetWidth;
  }
  ...
  private draw(analyser: AnalyserNode) {
    ...
    const drawVisualizer = () => {
      ...
      this.canvasCtx.strokeStyle = 'rgba(0, 87, 113, 1)';
  ...
  private startSilenceDetection() {
    ...
    this.silenceInterval = setInterval(() => this.checkSilence(), 100);
  }
