...
@Component({
  selector: 'app-text-classifier-settings-modal',
  templateUrl: './modal.component.html',
  styleUrls: ['./modal.component.scss'],
  standalone: true,
  imports: [IonicModule, TranslocoModule, FormsModule],
})
export class TextClassifierSettingsModalComponent {
  ...
  constructor() {
    ...
    const currentTextClassifier = this.llmConfig.getRecommendedModelForBrowser('textClassifier');
    this.selectedModel = currentTextClassifier?.id || '';
    this.initialModel = this.selectedModel;
  }

  filterModels(): void {
    const textClassifierModels = this.llmConfig.getModelsByType('textClassifier');
    this.models = textClassifierModels.map((model) => ({
      name: model.id,
      size: model.size,
    }));
  }
