...
export class DetectorService {
  ...
  public detectionSignal: WritableSignal<DetectionResult[] | null> = signal<DetectionResult[] | null>(null);
  ...
  private get detector(): unknown {
    return PipelineFactory.getExistingInstance('detector', this.detectorConfigStorage.model);
  }
  ...
  private async runDetection(imageData: ImageData | HTMLImageElement | HTMLCanvasElement): Promise<DetectionResult[] | null> {
    await this.createDetector();
    this.errorSignal.set(null);

    try {
      const result = await (this.detector as (input: unknown, options?: Record<string, unknown>) => Promise<unknown>)(
        imageData,
        {
  ...
  private async createDetector(): Promise<void> {
    try {
      ...
      const existingInstance = PipelineFactory.getExistingInstance('detector', this.detectorConfigStorage.model);
      if (existingInstance) {
        return; // Already loaded
      }

      // Load the detector using centralized pipeline factory
      await PipelineFactory.getInstance('detector', this.detectorConfigStorage.model, (data: unknown) => {
        ...
        switch (dataObj['status']) {
          case 'progress': {
            ...
            this.progressItemsSignal.update((items) =>
              items.map((item) =>
                (item as Record<string, unknown>)['file'] === (dataObj['file'] as string) ? { ...(item as Record<string, unknown>), progress: dataObj['progress'] } : item,
              ),
            );
            break;
          }
          ...
          case 'done': {
            ...
            this.progressItemsSignal.update((items) =>
              items.filter((item) => (item as Record<string, unknown>)['file'] !== (dataObj['file'] as string)),
            );
            break;
          }
        }
  ...
  private async tryFallbackModels(): Promise<void> {
    ...
    for (const fallbackModel of fallbackModels) {
      ...
      try {
        ...
        await PipelineFactory.disposeInstance('detector', currentModel);

        // Try loading the fallback model
        await PipelineFactory.getInstance('detector', fallbackModel, (data: unknown) => {
          const dataObj = data as Record<string, unknown>;
          switch (dataObj['status']) {
            case 'progress': {
              this.progressItemsSignal.update((items) =>
