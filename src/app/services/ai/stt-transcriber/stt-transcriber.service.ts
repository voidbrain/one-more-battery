...
export class TranscriberService {
  ...
  public progressItemsSignal: WritableSignal<whisperDownloadFilesProgress[]> = signal<
    whisperDownloadFilesProgress[]
  >([]);
  ...
  private get transcriber(): unknown {
    return PipelineFactory.getExistingInstance(
      'sttTranscriber',
      this.transcriberConfigStorage.model,
    );
  }
  ...
  async transcribe(audio: Float32Array): Promise<TranscriberData | null> {
    ...
    try {
      ...
      const output = await (
        this.transcriber as (audio: Float32Array, options: unknown) => Promise<unknown>
      )(audio, {
        top_k: 0,
        do_sample: false,
  ...
  private async tryFallbackModels(): Promise<void> {
    ...
    for (const fallbackModel of fallbackModels) {
      ...
      try {
        ...
        await PipelineFactory.disposeInstance('sttTranscriber', currentModel);

        // Try loading the fallback model
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await PipelineFactory.getInstance('sttTranscriber', fallbackModel, (data: any) => {
          switch (data.status) {
            case 'progress': {
              this.progressItemsSignal.update((items) =>
                items.map((item) =>
        ...
        if (isChromeProtobufError && this.isProtobufError(fallbackError)) {
          console.warn(
            'Chrome protobuf compatibility issue detected. Skipping remaining ONNX models.',
          );
          break;
        }
  ...
  private async handleModelLoadingFailure(isChromeProtobufError: boolean): Promise<void> {
    ...
    if (isChromeProtobufError) {
      errorMessage +=
        'Chrome has known compatibility issues with ONNX models. Try using Safari or Firefox for better transcription support.';
    } else {
  ...
  private createStreamer(): void {
    ...
    const transcriberInstance = this.transcriber as {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    this.streamer = new WhisperTextStreamer(transcriberInstance.tokenizer as any, {
