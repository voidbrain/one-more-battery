...
class WebmContainer extends WebmBase<ContainerData[]> {
  ...
  override updateBySource() {
    ...
    for (this.offset = 0; this.offset < this.source!.length; this.offset = end) {
      ...
      let ctr: new (name: string, type: string) => WebmBase<unknown> = WebmBase;
      switch (info.type) {
        case 'Container':
          ctr = WebmContainer;
          break;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      reader.addEventListener('loadend', () => {
        try {
          let result = reader.result as ArrayBufferLike;
          // Check if SharedArrayBuffer is available and result is instance of it
          if (typeof SharedArrayBuffer !== 'undefined' && result instanceof SharedArrayBuffer) {
            // Convert SharedArrayBuffer to ArrayBuffer
            const tempUint8Array = new Uint8Array(result);
            result = tempUint8Array.buffer.slice(0); // Create a copy to ensure it's an ArrayBuffer
          }
          const file = new WebmFile(new Uint8Array(result as ArrayBuffer));
